document.addEventListener('turbo:load', function() {
    const modal = document.getElementById('deleteModalDelivered');
    if (!modal) return; // Exit if modal is not found

    attachDeleteModalListenersDelivered();
...
function attachDeleteModalListenersDelivered() {
    const modal = document.getElementById('deleteModalDelivered');
    const closeModal = modal.querySelector('.close-button');
    const cancelModal = modal.querySelector('.cancel-button');
    const deleteLinks = document.querySelectorAll('.delete-link-delivered');
    const confirmDeleteButton = modal.querySelector('#confirmDeleteButtonDelivered');
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    if (tableBody && tableBody.children.length === 0) {
                        ...
                        noDataCell.textContent = 'No delivered deliveries found';
